...
<body  style="padding-top:0px;">
	...
	<div class="container"><!---HEADER-->
		<div class="row">
				<div class="col-lg-6">
					<div style="float:left;padding:5px 10px 5px 0px;"><img src="<?=base_url().$logo;?>" style='width:60px; vertical-align:middle;'></div>
...
function recSidebar($nav,$akv) {
    foreach ($nav as $key=>$val) {
		if(isset($val->anak)){
			...
			echo '<li '.$cActt.' style="margin:0px;padding:0px;"><a href="'.site_url().$val->path_menu.'" style="margin:5px;padding:5px;"><i class="fa fa-'.$val->icon_menu.' fa-fw"></i> '.$val->nama_menu.'</a></li>';
		} // end anak
    } // end foreach
} // end recKanal
?>
...
function loadSegment(segmen,page){
    ...
    $.sessionTimeout({
        keepAliveUrl: '<?=site_url();?>admin/lanjutkan',
