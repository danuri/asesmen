...
class Web_agenda extends MX_Controller {
	...
	public function read_side()	{
		...
		$data['arsip'] = $this->daftar_arsip();
		$data['daftar_kategori'] = $this->all_side();
...
//			$data['hslquery'][$key]->tgl_selesai=$this->m_agenda->c_tanggal($detil->tgl_selesai);
			$data['hslquery'][$key]->tgl_mulai=$detil->tgl_mulai;
			$data['hslquery'][$key]->tgl_selesai=$detil->tgl_selesai;
			$data['hslquery'][$key]->hari_mulai=$hh[$this->c_nama_hari($detil->tgl_mulai)];
			$data['hslquery'][$key]->hari_selesai=$hh[$this->c_nama_hari($detil->tgl_selesai)];
		}

		$de = Modules::run("crest/pager/pagerA",$dt['count'],$batas,$hal);
		$data['pager']=$de;
		echo json_encode($data);
	}
	...
	public function all($t_hal,$t_id_kat)	{
		...
		if(!$sess->id_kanal || $judulseo!=$this->uri->segment(5) || $er==FALSE){
			...
			$data['isi'][$key]->hari_selesai=$hh[$this->m_agenda->c_hari($detil->tgl_selesai)];
					...
					$data['isi'][$key]->thumb = (!empty($val->foto))?$val->foto:$def;
				}

				$pph=site_url().$this->uri->segment(1)."/".$this->uri->segment(2)."/".$this->uri->segment(3);
				$ddmm = Modules::run("crest/pager/pagerB",$dt['count'],$batas,$hal,5);
				$ganti=str_replace("XX",$pph,$ddmm);
				$ganti=str_replace("YY",$this->uri->segment(5),$ganti);
				$data['pager']=(empty($data['isi']))?"...":$ganti;
	
						$path='assets/themes/'.$sessm['theme'].'/komponen/web_agenda/all.php';
						if(file_exists($path)){	
							$this->viewPath = '../../assets/themes/'.$sessm['theme'].'/komponen/web_agenda/';
...
////////
	...
	public function c_nama_hari($tgl)	{
		$nama = $this->m_agenda->c_hari($tgl);
		return $nama;
	}
