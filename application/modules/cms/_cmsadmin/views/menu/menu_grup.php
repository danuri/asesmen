<div class="row">
	<div class="col-lg-12">
		<h3 class="page-header"><?=$setting;?>
		<div class="btn btn-warning btn-xs pull-right content_post" style="margin-top:10px;" onclick="loadForm('formtambah_menu_pengguna','xx','0','0');"><i class="fa fa-plus fa-fw"></i> Tambah Menu Pengguna</div>
		</h3>
	</div><!-- /.col-lg-12 -->
</div><!-- /.row -->
...
<!--
									<a href="<?=site_url('module/cmsadmin/sub_menu/menu_grup');?>" class="btn btn-default btn-xs pull-right">Sub Menu</a>
			...
			<div class="panel-heading" style="padding-top:2px;padding-bottom:5px;">
						<div class="row">
								<div class="col-lg-8" style="padding-top:8px;">
										<i class="fa fa-gear fa-fw"></i> Daftar Menu
								</div><!---/.col-lg-8 -->
...
////////////////tombol treexgrid && variabel kunci-->
				if(item.toggle == "tutup"){
					table = table+ "<td style='padding-left: "+ item.spare +"px;'><div style=\"float:left; padding:1px 5px 0px 0px;\"><i class=\"fa treex fa-chevron-circle-right fa-fw\" style=\"font-size:16px; cursor: pointer;\" data-expand=\"no\"  id='"+item.idchild+"' onclick=\"loadIsiGrid('"+item.idchild+"','"+item.level+"');\"></i></div><div>"+item.nama_item+ "</div></td>";
				} else {
...
$(document).on('click', '.fa.treex',function(){
	...
	if(lvl=='yes'){
		$("[id^='row_"+idp+"_']").hide();
		$("#"+idp).replaceWith("<span class=\"fa treex fa-chevron-circle-right fa-fw\" style=\"font-size:16px; cursor: pointer;\" data-expand=\"no\"  id='"+idp+"'></span>");
	} else {
