<div class="row">
	<div class="col-lg-12">
		<h3 class="page-header">Menu
		<div class="btn btn-warning btn-xs pull-right content_post" style="margin-top:10px;" onclick="loadForm('formtambah_menu','xx','0','0');"><i class="fa fa-plus fa-fw"></i> Tambah Menu</div>
		</h3>
	</div><!-- /.col-lg-12 -->
</div><!-- /.row -->
	...
	<div class="col-lg-12">
		<div class="panel panel-warning" id="panel_utama">
			<div class="panel-heading">
						<div class="row">
								<div class="col-lg-6">
									<i class="fa fa-gears fa-fw"></i> Daftar Menu
								</div><!---/.col-lg-6 -->
								<div class="col-lg-6" style="text-align:right;">
									<div class="btn btn-warning btn-xs">Menu Utama</div>
									<a href="<?=site_url('module/cmsadmin/sub_menu');?>" class="btn btn-default btn-xs">Sub Menu</a>
								</div><!---/.col-lg-6 -->
						</div><!---/.row -->
			</div>
...
//tombol aksi<--
				table = table+ "</td>";
////////////////tombol treexgrid && variabel kunci-->
				if(item.toggle == "tutup"){
					table = table+ "<td style='padding-left: "+ item.spare +"px;'><div style=\"float:left; padding:1px 5px 0px 0px;\"><i class=\"fa fa-chevron-circle-right fa-fw treex\" style=\"cursor: pointer;\" data-expand=\"no\"  id='"+item.idchild+"' onclick=\"loadIsiGrid('"+item.idchild+"','"+item.level+"');\"></i></div><div>"+item.nama_item+ "</div></td>";
				} else {
					table = table+ "<td style='padding-left: "+ item.spare +"px;'><div style=\"float:left; padding:1px 5px 0px 0px;\"><i class=\"fa fa-file-o fa-fw\" data-expand=\"no\"  id='"+item.idchild+"'></i></div><div>"+item.nama_item+ "</div></td>";
				}
////////////////tombol treexgrid && variabel kunci<--
		...
		if(lvl == 0){
			...
			$("#"+idp).replaceWith("<span class=\"fa treex fa-chevron-circle-down fa-fw\" style=\"font-size:16px; cursor: pointer;\" data-expand=\"yes\"  id='"+idp+"'></span>");
		}
...
////////////////////////////////////////////////////////////////////////////
$(document).on('click', '.fa.treex',function(){
	var lvl = $(this).attr("data-expand");
	var idp = $(this).attr("id");
	if(lvl=='yes'){
		$("[id^='row_"+idp+"_']").hide();
		$("#"+idp).replaceWith("<span class=\"fa fa-chevron-circle-right fa-fw treex\" style=\"cursor: pointer;\" data-expand=\"no\"  id='"+idp+"'></span>");
	} else {
		$("[id^='"+idp+"_']").each(function(key,val) {
			var ini = $(this).attr("id");
			var status_ini = $(this).attr("data-expand");
			$("#row_"+ini+"").show();
			if(status_ini == "yes"){	$(this).removeClass("fa treex fa-chevron-circle-down fa-fw").addClass("fa treex fa-chevron-circle-right fa-fw").attr("data-expand","no");	}
		});
		$("[id^='"+idp+"_']").each(function(key,val) {	var ini = $(this).attr("id");	$("[id^='row_"+ini+"_']").hide();	});
		$("#"+idp).replaceWith("<span class=\"fa treex fa-chevron-circle-down fa-fw\" style=\"cursor: pointer;\" data-expand=\"yes\"  id='"+idp+"'></span>");
	}
});
////////////////////////////////////////////////////////////////////////////
