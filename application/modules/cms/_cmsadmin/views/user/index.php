<div class="row">
	<div class="col-lg-12">
		<h3 class="page-header">Pengguna
		<div class="btn btn-warning btn-xs pull-right content_post" style="margin-top:10px;" onclick="loadForm('formtambah','xx');"><i class="fa fa-plus fa-fw"></i> Tambah Pengguna</div>
		</h3>
	</div><!-- /.col-lg-12 -->
</div><!-- /.row -->
			...
			<div class="panel-heading" style="padding-top:2px;padding-bottom:5px;">
						<div class="row">
								<div class="col-lg-8" style="padding-top:8px;">
										<i class="fa fa-user fa-fw"></i> Daftar Pengguna
								</div><!---/.col-lg-8 -->
...
var grup = $('#group_pil').val();
			$.ajax({
				...
				data:{"hal": hal, "batas": batas,"cari":cari, "grup": grup},
...
//tombol aksi-->
						...
						if(item.status=='on'){
							table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="loadForm(\'formnonaktifkan\',\''+ item.user_id +'\');\"><i class="fa fa-bell-slash-o fa-fw"></i> Non-Aktifkan</a></li>';
						} else {
							table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="loadForm(\'formaktifkan\',\''+ item.user_id +'\');\"><i class="fa fa-bell-o fa-fw"></i> Aktifkan</a></li>';
						}
						table = table+ "</ul>";
						table = table+ "</div>";
//tombol aksi<--
				...
				table = table+ "<td><div id='group_name_"+item.user_id+"'>" +item.group_name+"</div></td>";
					if(item.status=='on'){
						table = table+ '<td><div class="btn btn-primary btn-xs"><i class="fa fa-check fa-fw"></i> On</div></td>';
					} else {
						table = table+ '<td><div class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i> Off</div></td>';
					}
				table = table+ "</tr>";       
			no++;
			j++;
