...
//////////////////////////////////////////////////////////////////////////////////
	function getuser($mulai,$batas,$cari,$path){
		if($path=="xx"){$and1="";}else{$and1=" AND a.group_id='$path'";}
		$sqlstr="SELECT a.*,b.nama_item AS group_name 
		FROM users a LEFT JOIN (cmf_setting b) ON (a.group_id=b.id_item) 
		WHERE (username LIKE '$cari%' OR nama_user LIKE '$cari%')
		$and1 
		ORDER BY b.id_item ASC,a.user_id ASC  LIMIT $mulai,$batas";
		$hslquery=$this->db->query($sqlstr);
		return $hslquery;
	}
	function hitung_user($path,$cari){
		if($path=="xx"){$and1="";}else{$and1=" AND group_id='$path'";}

		$query=$this->db->query("SELECT count(user_id) as count_nik FROM users 
		WHERE (username LIKE '$cari%' OR nama_user LIKE '$cari%')
		$and1"); 
		$row = $query->row_array();
    ...
    function aktifkan_user_aksi($idg,$ipp){
		$sqlstr="UPDATE users SET status='on' WHERE user_id='$idg'";
		$this->db->query($sqlstr);
		$hasil = "sukses";
		return $hasil;
	}
    function nonaktifkan_user_aksi($idg,$ipp){
		$sqlstr="UPDATE users SET status='off' WHERE user_id='$idg'";
		$this->db->query($sqlstr);
		$hasil = "sukses";
		return $hasil;
	}
