...
*/
    ...
    function tambah_aksi($ipp){
		...
		if($ipp['level']==0){
			...
			$logo = $this->getopsivalue_all('logo_app');
			...
			$jj = json_decode($logo->meta_value);
			$logonya = $jj->nilai;
			$sqlstr="INSERT INTO cmf_setting (id_item,nama_item,id_setting,id_parent,meta_value) VALUES ('$maxidB','$logonya','10','".$logo->id_parent."','$ini')";
			$this->db->query($sqlstr);
			$sqlstr="UPDATE konten_appe SET nilai=(nilai+1) WHERE id_appe='".$logo->id_parent."'";
			$this->db->query($sqlstr);
	}
    ...
    function hapus_kanal_aksi($ipp){
		...
		$sqA="SELECT * FROM cmf_setting WHERE id_setting='10' AND meta_value LIKE '%\"id_kanal\":\"".$ipp['idk']."\"%'";
		$hslA=$this->db->query($sqA)->row();
		
		$sqF="UPDATE konten_appe SET nilai=(nilai-1) WHERE id_appe='".$hslA->id_parent."'";
		$this->db->query($sqF);
...
//////////////////////////////////////////////////////////////////////////////////
	...
	function getopsivalue_all($nama){
...
*/
		$sqA="SELECT nama_item,meta_value FROM cmf_setting WHERE id_setting='10' AND meta_value LIKE '%\"id_kanal\":\"".$idd."\"%'";
		$query=$this->db->query($sqA)->row();
		return $query;
	}
    ...
    function simpan_logo_kanal($id_kanal,$id_gambar,$nama_file){
		$sq="SELECT * FROM konten_appe WHERE id_appe='$id_gambar'";
		$hs=$this->db->query($sq)->row();
		if($hs->keterangan_appe=="jpg" || $hs->keterangan_appe=="JPG" || $hs->keterangan_appe=="gif" || $hs->keterangan_appe=="GIF" || $hs->keterangan_appe=="png" || $hs->keterangan_appe=="PNG"){
			$sqA = "SELECT * FROM cmf_setting WHERE id_setting='10' AND meta_value LIKE '%\"id_kanal\":\"".$id_kanal."\"%'";
			$qrA = $this->db->query($sqA)->row();
	
			$sqlstr="UPDATE cmf_setting SET nama_item='$nama_file',id_parent='$id_gambar' WHERE id_item='".$qrA->id_item."'";		
			$this->db->query($sqlstr);
			$sqlstr="UPDATE konten_appe SET nilai=(nilai+1) WHERE id_appe='$id_gambar'";
			$this->db->query($sqlstr);
			$sqlstr="UPDATE konten_appe SET nilai=(nilai-1) WHERE id_appe='".$qrA->id_parent."'";
			$this->db->query($sqlstr);
		}
	}
//////////////////////////////////////////////////////////////////////////////////
...
//		$sqA = "SELECT * FROM cmf_setting WHERE id_setting='9' AND meta_value LIKE '%\"id_kanal\":\"".$idd."\"%' ORDER BY urutan";
//		$hslquery = $this->db->query($sqA)->result();
...
/*
		...
		$hslquery = $this->db->get()->row();
*/
		$sqA="SELECT id_item,meta_value FROM cmf_setting WHERE id_setting='11' AND nama_item='$posisi' AND meta_value LIKE '%\"id_kanal\":\"".$idk."\"%'";
		$hslquery=$this->db->query($sqA)->row();
	...
	function getwrapper($path,$posisi){
		...
		$hslquery = $this->db->get()->row();
/*
		$sqA="SELECT meta_value FROM cmf_setting WHERE id_setting='11' AND nama_item='$posisi' AND meta_value LIKE '%\"path_kanal\":\"".$path."\"%'";
		$hslquery=$this->db->query($sqA)->row();
*/
