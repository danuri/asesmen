...
?>
				...
				<input type="text" id="label_<?=($key+1);?>" name="label[]" value="<?=$val->judul_appe;?>" class="ipt_text" style="width:400px;float:left;" onblur="editAtr('<?=($key+1);?>','<?=$jj[$i]->id_appe;?>');">
				<?php if($jml>1){ ?>
				<div class="btn btn-default btn-xs" onclick="hapus_atr('<?=$jj[$i]->id_appe;?>');" title='Klik untuk menghapus' style="float:left;margin-left:5px;"><i class="fa fa-trash fa-fw"></i></div>
				<?php } ?>
				<?php if($i!=0){ ?>
...
function urut_atr(id_ini,urutan_ini,id_lawan,urutan_lawan){
	...
	function hapus_atr(id_ini){
				$.ajax({
					type:"POST",
					url:"<?=site_url();?>cmskonten/direktori/hapus_atribut",
					beforeSend:function(){	 },
					data:{"id_ini": id_ini},
					success:function(data){	
						loadForm3('formeditkategori/tidak','<?=$idd;?>','kategori');	
	...
	function editAtr(no,id_ini){
				var baru = $("#label_"+no).val();
				$.ajax({
					type:"POST",
					url:"<?=site_url();?>cmskonten/direktori/edit_atribut",
					beforeSend:function(){	 },
					data:{"id_ini": id_ini,"nilai":baru},
					success:function(data){	
						loadForm3('formeditkategori/tidak','<?=$idd;?>','kategori');	
					}, 
				dataType:"html"});
