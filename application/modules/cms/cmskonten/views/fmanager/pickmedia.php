...
////////////////////////////////////////////////////////////////////////////
function loadIsiGridFml(idp,lvl){
var mulai=0;
			$.ajax({
				type:"POST",
		...
		if((data.isi.length)>0){
			...
			$.each( data.isi, function(index, item){
				...
				if(item.banyak_file==0){	var banyak = item.judul_appe;	} else {	var banyak = item.judul_appe+' <div class="btn btn-warning btn-xs" onClick="loadBackF(\'pilihmedia\',\''+item.idchild +'\','+(parseInt(lvl)+1)+',\''+item.idchild+'\');">: '+item.banyak_file+'</div>';	}
////////////////tombol treexgrid && variabel kunci-->
				if(item.toggle == "tutup"){
					table = table+ "<td style='padding-left: "+ item.spare +"px;padding-top:1px;padding-bottom:0px;'><div style=\"float:left; padding:1px 5px 0px 0px;\"><i class=\"fa treex fa-folder fa-fw\" style=\"font-size:16px; cursor: pointer;\" data-expand=\"no\"  id='"+item.idchild+"' onclick=\"loadIsiGridFml('"+item.idchild+"','"+item.level+"');\"></i></div>"+banyak+ "</td>";
				} else {
...
////////////////tombol treexgrid && variabel kunci<--
		...
		if(lvl == 0){
			...
			$("#"+idp).replaceWith("<span class=\"fa treex fa-folder-open fa-fw\" style=\"font-size:16px; cursor: pointer;\" data-expand=\"yes\"  id='"+idp+"'></span>");
		}
...
$(document).on('click', '.fa.treex',function(){
	...
	if(lvl=='yes'){
		$("[id^='row_"+idp+"_']").hide();
		$("#"+idp).replaceWith("<span class=\"fa treex fa-folder fa-fw\" style=\"font-size:16px; cursor: pointer;\" data-expand=\"no\"  id='"+idp+"'></span>");
	} else {
		$("[id^='"+idp+"_']").each(function(key,val) {
			var ini = $(this).attr("id");
			var status_ini = $(this).attr("data-expand");
			$("#row_"+ini+"").show();
			if(status_ini == "yes"){	$(this).removeClass("fa treex fa-folder-open fa-fw").addClass("fa treex fa-folder fa-fw").attr("data-expand","no");	}
		});
...
////////////////////////////////////////////////////////////////////////////
function loadBackF(tujuan,idd,level,idparent){	
	$("#formPost").html('');
	$("#gridPost").hide();
			$.ajax({
				type:"POST",
