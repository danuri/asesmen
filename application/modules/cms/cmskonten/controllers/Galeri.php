...
class Galeri extends MX_Controller {
	...
	function edit_aksi(){
 		$this->form_validation->set_rules("idd","IDD","trim|required");
 		$this->form_validation->set_rules("judul","Judul Berita Foto","trim|required");
        $this->form_validation->set_rules("id_kategori","Rubrik Berita Foto","trim|required");
 		$this->form_validation->set_rules("tgl_buat","Tanggal Berita Foto","trim|required");
	...
	function tambah_aksi(){
        ...
        $this->form_validation->set_rules("id_kategori","Rubrik Berita Foto","trim|required");
 		...
 		$this->form_validation->set_rules("keterangan","Keterangan","trim|required");
		if($this->form_validation->run()) {
			$this->m_galeri->tambah_galeri_aksi($_POST); 
			echo "sukses#"."add#";
		 } else {
			echo "error-".validation_errors()."#0";	
		 }
