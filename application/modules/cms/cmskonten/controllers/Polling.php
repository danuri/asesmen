...
class Polling extends MX_Controller {
	...
	function getkonten(){
		...
		if($dt==0){
			...
			$data['pager'] = Modules::run("crest/pager/pagerC",$dt,$batas,$hal);
		}
	...
	function tambah_aksi(){
		...
		$this->form_validation->set_rules("judul","Judul Polling","trim|required");
...
//////////////////////////PILIHAN HANDLING//////////////////////////
	...
	function tambah_pilihan_aksi(){
		...
		for($i=0;$i<$jml;$i++){
	        $this->form_validation->set_rules("pilihan[$i]","Pilihan - ".($i+1),"trim|required");
		}
		if($this->form_validation->run()) {
...
/*
function loadForm(){
	$('#sb_act').attr('action','<?=site_url();?>module/cmskonten/polling/formpilihanedit');
