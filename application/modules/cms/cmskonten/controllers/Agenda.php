...
class Agenda extends MX_Controller {
	...
	function tambah_aksi(){
 		$this->form_validation->set_rules("judul","Judul Agenda","trim|required");
        $this->form_validation->set_rules("id_kategori","Rubrik Agenda","trim|required");
 		$this->form_validation->set_rules("sub_judul","Tempat","trim|required");
	...
	function formedit(){
		...
		$data['detil'] = json_decode($data['isi']->isi_konten);
				$vv = "\n<select id='id_kategori' name='id_kategori' class=\"form-control\">\n<option value=''>-- Pilih --</option>\n";
				$vv = $vv.Modules::run("cmshome/kategori_options",@$data['isi']->id_kategori,"agenda");
				$vv = $vv."</select>\n";
		$data['pilrb'] = $vv;
		$data['penulis_options'] =  Modules::run("cmshome/penulis_options",@$data['isi']->id_penulis);
		$data['aksi'] = "edit";
