...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	function tambah_folder_aksi(){
		...
		if(empty($cek)){
			...
			$this->m_fmanager->tambah_folder_aksi($id_parent,$ipp,$pth,$this->user_id);
		}
		echo "sukses#"."add#";
	}
	...
	function edit_folder_aksi(){
		...
		if(empty($cek)){
			...
			$this->m_fmanager->edit_folder_aksi($id_ini,$ipp,$pth,$this->user_id);
		}
		echo "sukses#"."add";
	}
		...
		if(strlen($_FILES['artikel_file']['name'])>0){
			...
			$komponen = $_POST['path'];
				...
				if(!file_exists($komponen.$nama_file)){
							...
							////////////////////////////////
							$nmF = $result['raw'].".".$ext;
							rename($komponen.$base.$nmF,$komponen.$nama_file);
							////////////////////////////////
...
//		$config['max_size']	= '512';
		...
		if ( ! $this->upload->do_upload('artikel_file'))		{
			...
			$ttpp = $dn['raw_name'];
			$this->m_fmanager->simpan_file($id_konten,$nama_file,$ext,$this->user_id);
			$data['raw'] = $ttpp;
			$data['status'] = "success";
			$data['idd'] = $id_konten;
			return $data;
	...
	function cek_hapus(){
		...
		$cari = $path.$file;
		$sqlstr="SELECT a.id_konten FROM konten a WHERE a.isi_konten LIKE '%$cari%' OR a.judul LIKE '%$cari%'";
	...
	function formfile_rename(){
		...
		if(!file_exists($_POST['path'].$nama_baru.".".$ini->keterangan_appe)){
			...
			$this->m_fmanager->rename_fl($_POST['idd'],$baru,$this->user_id);
		}
		echo "sukses#"."add#";
	}
