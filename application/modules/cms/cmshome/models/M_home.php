...
//////////////////////////////////////////////////////////////////////////////////
    ...
    function simpan_file($idd,$idg,$nama){
		$sq="SELECT * FROM konten_appe WHERE id_appe='$idg'";
		...
		if($hs->keterangan_appe=="jpg" || $hs->keterangan_appe=="JPG" || $hs->keterangan_appe=="gif" || $hs->keterangan_appe=="GIF" || $hs->keterangan_appe=="png" || $hs->keterangan_appe=="PNG"){
			...
			$inj="{\"label\":\"".$label."\",\"nilai\":\"".$nama."\",\"tipe\":\"".$tipe."\"}";
			$sqlstr="UPDATE cmf_setting SET meta_value='$inj',id_parent='$idg' WHERE id_item='".$idd."'";
			$this->db->query($sqlstr);

			$sqlstr="UPDATE konten_appe SET nilai=(nilai+1) WHERE id_appe='$idg'";
			$this->db->query($sqlstr);
			$sqlstr="UPDATE konten_appe SET nilai=(nilai-1) WHERE id_appe='".$ini[0]->id_parent."'";
			$this->db->query($sqlstr);
			return $ini[0]->id_parent;
		} else {
			return "0";
		}
