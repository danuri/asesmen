...
/*	
	function index(){
		$data['data_simsdm'] = $this->M_peserta->data_simsdm();
		$this->load->view('peserta/peserta',$data);
	}
*/	

	function index(){
		$data['data_peserta'] = $this->M_peserta->data_peserta();
		$this->load->view('peserta/peserta',$data);
	}
...
/*	
	function data_peserta(){
		$data = $this->M_peserta->data_peserta();
		echo json_encode($data);
	}	
*/	
	function data_peserta_sjt(){
		$data = $this->M_peserta->data_peserta_sjt();
		echo json_encode($data);
	}	
	
	function data_simsdm(){
		$data = $this->M_peserta->data_simsdm();
		echo json_encode($data);
	}	
	
	function get_internal($idd){
		
		$data['identitas'] 	= $this->M_peserta->get_identitas($idd);
		$data['r_jab'] 		= $this->M_peserta->get_r_jab($idd);
		$data['assessment'] = $this->M_peserta->get_assessment($idd);
		
		$data['hasil_ci']	= $this->M_peserta->get_ci($idd);
		$data['apm']		= $this->M_peserta->lihat_apm_dua_row($idd);
		$data['cfit']		= $this->M_peserta->lihat_cfit($idd);
		
		$data['kegiatan']	= $this->M_peserta->get_kegiatan($idd);
		$data['hasil'] 		= $this->M_peserta->get_detail_hasil($idd);
		$data['penilaian']	= $this->M_peserta->get_penilaian($idd);
		$data['detail_lap']	= $this->M_peserta->detail_lap($idd);
	...
	function get_peserta_sjt($idd){
		$data['profile'] 	= $this->M_peserta->get_profile_sjt($idd);
		$data['jabatan'] 	= $this->M_peserta->get_jabatan_sjt($idd);
		$data['assessment'] = $this->M_peserta->get_assessment_sjt($idd);
		$data['kompetensi'] = $this->M_peserta->kompetensi_sjt($idd);			
		...
		$data['potensi']	= $this->M_peserta->get_potensi_kerjasama($idd);
		*/
		$this->load->view('peserta/detail_peserta_sjt', $data);
    }	
	...
	function lihat_papi($idd){
		$data['papi'] = $this->M_peserta->cetak_papi_row($idd);
		$this->load->view('cetak/lap_papi', $data);
	}	
...
//// END PEGAWAI MASUK ////		
	...
	function update_kom_lgd(){		
		...
		$data				=$this->M_peserta->update_kom_lgd_kerjasama($nip,$nilai_lgd_kjs,$bobot_lgd_kjs,$ev_lgd_kjs,$nilai_lgd_kom,$bobot_lgd_kom,$ev_lgd_kom,$nilai_lgd_oph,$bobot_lgd_oph,$ev_lgd_oph,$nilai_lgd_mp,$bobot_lgd_mp,$ev_lgd_mp,$nilai_lgd_pk,$bobot_lgd_pk,$ev_lgd_pk);
		echo json_encode($data);
		redirect($_SERVER['HTTP_REFERER']);
    }	
	
	function update_kom_ankas(){		
		$nip				=$this->input->post('nip');
		...
		$data				=$this->M_peserta->update_kom_ankas_kerjasama($nip,$nilai_ankas_kom,$bobot_ankas_kom,$ev_ankas_kom,$nilai_ankas_oph,$bobot_ankas_oph,$ev_ankas_oph,$nilai_ankas_pp,$bobot_ankas_pp,$ev_ankas_pp,$nilai_ankas_mp,$bobot_ankas_mp,$ev_ankas_mp,$nilai_ankas_pk,$bobot_ankas_pk,$ev_ankas_pk);
		echo json_encode($data);
		redirect($_SERVER['HTTP_REFERER']);
    }	
	
	function update_kom_pw(){		
		$nip			=$this->input->post('nip');
		...
		$data			=$this->M_peserta->update_kom_pw_kerjasama($nip,$nilai_pw_int,$bobot_pw_int,$ev_pw_int,$nilai_pw_pdol,$bobot_pw_pdol,$ev_pw_pdol,$nilai_pw_pb,$bobot_pw_pb,$ev_pw_pb);
		echo json_encode($data);
		redirect($_SERVER['HTTP_REFERER']);
    }	
	
	function update_kom_bei(){		
		$nip			=$this->input->post('nip');
		...
		$data			=$this->M_peserta->update_kom_bei_kerjasama($nip,$nilai_bei_int,$bobot_bei_int,$ev_bei_int,$nilai_bei_kjs,$bobot_bei_kjs,$ev_bei_kjs,$nilai_bei_kom,$bobot_bei_kom,$ev_bei_kom,$nilai_bei_oph,$bobot_bei_oph,$ev_bei_oph,$nilai_bei_pp,$bobot_bei_pp,$ev_bei_pp,$nilai_bei_pdol,$bobot_bei_pdol,$ev_bei_pdol,$nilai_bei_mp,$bobot_bei_mp,$ev_bei_mp,$nilai_bei_pk,$bobot_bei_pk,$ev_bei_pk,$nilai_bei_pb,$bobot_bei_pb,$ev_bei_pb);
		echo json_encode($data);
		redirect($_SERVER['HTTP_REFERER']);
    }	
	
	function update_kom_bei_setda(){		
		$nip			=$this->input->post('nip');
...
//// KOMPETENSI ////	
	...
	function get_cetak_hasil($idd){
		$data['cetak_hasil']= $this->M_peserta->cetak_hasil($idd);
		$this->load->view('cetak/cetak_hasil', $data);
    }	
