...
class Export_sjt extends MX_Controller {
	...
	$employee_data = $this->M_peserta->export_sjt();
...
/*		
	...
	$fileName 	 = 'apm2_';
      ...
      $employee_data = $this->M_pantau_ujian->export_apm2();
	  $rowCount = 5;
      foreach($employee_data as $row)
      {
		$objPHPExcel->setActiveSheetIndex(0)
                            ->setCellValue('B'.$rowCount, $row->id_peserta_apm_dua);
                            ->setCellValue('C'.$rowCount, $row->nama);
                            ->setCellValueExplicit('D'..$rowCount, $row->nip, PHPExcel_Cell_DataType::TYPE_STRING);
							->setCellValue('E'.$rowCount, $row->jwb_sjt1);
                            ->setCellValue('F'.$rowCount, $row->jwb_sjt2);
                            ->setCellValue('G' . $rowCount, $row->jwb_sjt3);
